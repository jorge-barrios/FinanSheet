...
### Manejo de Monedas y Reactividad

**SIEMPRE usar `useCurrency`** para garantizar la reactividad ante cambios de tasas. El hook implementa `useSyncExternalStore` para asegurar que todos los componentes se actualicen simultáneamente.

```typescript
import { useCurrency } from "../hooks/useCurrency";

// El hook se suscribe automáticamente a cambios en CurrencyService
const { convertAmount, getFxRateToBase } = useCurrency();
```

**NO** usar `useEffect` manuales para suscribirse a `CurrencyService`, ya que pueden causar condiciones de carrera o actualizaciones perdidas.

### Cálculo de Valores de Despliegue (Grid vs Modal)

**SIEMPRE usar `useCommitmentValue`** para calcular el valor a mostrar de un commitment, asegurando consistencia entre el Grid y los Modales.
...
// Calcula el valor correcto (histórico si pagado, live si pendiente)
// targetCurrency por defecto es 'CLP'
const value = getDisplayValue(amount, currency, payment, targetCurrency);
```

**Regla de Oro:** Si un componente muestra un valor monetario de un commitment, DEBE usar este hook. No reimplementar la lógica de "si está pagado usa X, si no usa Y".
...
const formInputClasses =
  "h-[36px] bg-white dark:bg-slate-700 border-slate-300 dark:border-slate-500 rounded-xl text-sm focus:ring-2 focus:ring-sky-500";

// Labels (11px uppercase)
const formLabelClasses =
  "text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5";
```
