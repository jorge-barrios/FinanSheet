...
Usage:
    python3 perspective.py --step 1 --total-steps 2 --perspective minimal
"""
...
def format_step_1(perspective_id: str, script_path: str) -> str:
    ...
    parts = [
        ...
        "",
        format_invoke_after(
            f"python3 {script_path} --step 2 --total-steps 2 --perspective {perspective_id}"
        ),
    ]
    return "\n".join(parts)
...
def format_output(
    step: int, total_steps: int, perspective: str, script_path: str
) -> str:
    """Format output for the given step."""
    if step == 1:
        return format_step_1(perspective, script_path)
    else:
...
def main():
    ...
    print(format_output(args.step, args.total_steps, args.perspective, script_path))
