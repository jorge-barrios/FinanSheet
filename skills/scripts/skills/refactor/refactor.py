...
DIMENSIONS_MODULE_PATH = "skills.refactor.dimensions"
...
def format_parallel_dispatch() -> str:
    ...
    lines.append("")
    lines.append(f'    Start: <invoke working-dir=".claude/skills/scripts" cmd="python3 -m {EXPLORE_MODULE_PATH} --step 1 --total-steps 2 --dimension $DIMENSION_ID" />')
    lines.append("  </template>")
    lines.append("")
...
def format_dimension_retrieval() -> str:
    ...
    lines.append(f'    <invoke working-dir=".claude/skills/scripts" cmd="python3 -m {DIMENSIONS_MODULE_PATH} --dimension $DIM_ID" />')
    lines.append("")
    lines.append("  Example for 3 selected dimensions (naming, architecture, types):")
    lines.append(f'    <invoke working-dir=".claude/skills/scripts" cmd="python3 -m {DIMENSIONS_MODULE_PATH} --dimension naming" />')
    lines.append(f'    <invoke working-dir=".claude/skills/scripts" cmd="python3 -m {DIMENSIONS_MODULE_PATH} --dimension architecture" />')
    lines.append(f'    <invoke working-dir=".claude/skills/scripts" cmd="python3 -m {DIMENSIONS_MODULE_PATH} --dimension types" />')
    lines.append("")
...
def format_output(step: int, total_steps: int) -> str:
    ...
    if step == 3:
        parts.append(format_dimension_retrieval())
        parts.append("")
