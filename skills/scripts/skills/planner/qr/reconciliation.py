...
def get_step_guidance(step: int, total_steps: int, module_path: str, **kwargs) -> dict:
    ...
    if step == 1:
        return {
            ...
            "next": f"python3 -m {module_path} --step 2 --total-steps {total_steps} --milestone {milestone}",
        }
    ...
    if step == 2:
        return {
            ...
            "next": f"python3 -m {module_path} --step 3 --total-steps {total_steps} --milestone {milestone}",
        }
...
GOOD (neutral): "What synchronization primitive is used?" -> discovers reality
        ...
        return {
            ...
            "next": f"python3 -m {module_path} --step 4 --total-steps {total_steps} --milestone {milestone}",
        }
