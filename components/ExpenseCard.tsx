...
const ExpenseCard: React.FC<ExpenseCardProps> = ({ expense, paymentStatus, currentYear, onEditExpense, onDeleteExpense, onOpenCellEditor }) => {
    ...
    const statusText = isPaid ? t('grid.paid') : isOverdue ? t('grid.overdue') : t('grid.unpaid');
    const statusColor = isPaid ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300' : isOverdue ? 'bg-rose-100 dark:bg-rose-500/20 text-rose-700 dark:text-rose-300' : 'bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-300';
    ...
    return (
        ...
        >
            ...
            <div className="flex justify-between items-center mt-2">
                <span className="text-2xl font-mono font-semibold text-teal-600 dark:text-teal-300">{format(amountInBase)}</span>
                {isInCurrentMonth ? (
                    <div className="flex items-center">
